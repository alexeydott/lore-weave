...
| **Changing the agent workflow itself, or personalising it** | [Agent Workflow Standard](./agent-workflow.md) |
...
| **Writing a doc, comment, commit message — anything persisted** | [Artifact Language](#a-platform-build-standards) — **artifacts are ENGLISH**; chat with the user is not. Quote the *meaning*, never paste the original. **ENFORCED** by `doc-language-gate.py` |
...
| **Agent Workflow Standard** | the three layers (project rules / agent runner / personal `.local`) and their precedence; the `aif-*` skill trees are GENERATED and installed 3× — never hand-edit one copy; adoption of AI Factory's `aif-gate-result` contract as the shape for LoreWeave gate output | [docs/standards/agent-workflow.md](./agent-workflow.md) | ACTIVE | `scripts/agent-skills-parity.py` (pre-commit + CI, shrink-checked quirk list) + `scripts/gate-wiring-gate.py` |
...
| **Artifact Language (LOCKED 2026-07-31)** | every persisted artifact is written in **English** — design/architecture/spec docs, handoffs, ADRs, code+comments+docstrings, identifiers, log/error messages, commit messages, test names. **Chat with the user is not an artifact** and stays in whatever language they wrote in. The failure mode is **MIXING**, not translation: an English doc with pasted-in Vietnamese fragments (usually a verbatim quote) — quote the *meaning* in English instead. Non-English allowed only where the text **IS the subject matter**: corpus/fixture content and cited spans, i18n bundles, CJK/RTL test data, domain terms with no English equivalent (glossed on first use). **Distinct from — and not in tension with — [Multilingual](./multilingual.md)**: that one forbids English-only *rule logic* at runtime; this one governs the *prose we author*. | [CLAUDE.md → Key Rules](../../CLAUDE.md) | **ACTIVE / ENFORCED** | `doc-language-gate.py --staged` (pre-commit) — judges **added lines only** (468/1962 tracked docs carry legacy Vietnamese; blocking on contents would switch the gate off within a day). `--all` reports the baseline, never blocks. Escape hatch must carry a reason or it is itself a finding. |
...
> ⚠️ **The "Wiring" column below is a CLAIM, and on 2026-07-29 some of it was stale.** Two rows named a
> workflow leg for a gate CI did not name, and six gates that this column implied were healthy had been
> **failing on `main` for four days**. Presence in `scripts/` and a row in this table are not evidence
> that a gate runs, let alone that it passes. `scripts/gate-wiring-gate.py` (below) is what turns this
> column from prose into a check — run it rather than reading this table.
...
| [scripts/closed-set-gate.py](../../scripts/closed-set-gate.py) | **Closed set vs its companion list** — `const NAME: [Enum; _]` must list every variant the enum declares (missing · unknown · duplicate). Root cause it guards: Rust forces you to *handle* every variant (a wildcard-free `match`) but cannot force an *array* to *contain* every variant, and `variant_count` is unstable — so the list drifts **silently**. It shipped once: `ModifierSource::ALL` listed 3 of 6, so `Base`/`Archetype`/`Lex` **flat** modifiers were constructible, accepted and discarded, and a world rule worked or did nothing depending on the author's choice of operator (XST-D7). Deterministic, so replay agreed with itself and all 76 tests stayed green. Partial list ⇒ `closed-set-gate: ok — <reason>` in its doc block | pre-commit `--staged`; self-tested by `--self-test` |
| [scripts/hot-path-gate.py](../../scripts/hot-path-gate.py) | **No string-keyed lookup on the island step path** (IMP-D4 / QTY-A9 / IMP-A3). Two checks, both on the **KEY TYPE** and never on the container: a `HashMap`/`BTreeMap`/`HashSet` keyed by a string type in a hot-path file, and a `.get("…")`/`.contains_key("…")`/`["…"]` read — caught even when the declaration lives elsewhere. Root cause it guards: `Q1` introduces author-declared quantities that arrive as **names** in TOML, and the shortest path to a working `Q2` is `resources: BTreeMap<String, ResourceState>` on the actor — after which `evaluate_outcome`'s closures each do a string compare + tree descent and `outcome_of` runs them **five times per call**, on every landed Strike and every `EndTurn`. **Determinism survives** (`BTreeMap` is ordered), so no existing test reds, and QTY-A9's letter is even satisfied because it forbids map iteration in *aggregation* — the bug is invisible to the suite by construction. Doc 26 anticipated the hard part and demanded *"this distinction must be **encoded**, not assumed"*: keying the rule on the key type is that encoding — `BTreeMap<EntityId, Actor>` structurally cannot match. Scope is the step path plus `ruleset-core` (read by reference inside `apply`); `ruleset-loader` is deliberately excluded — it is cold by IMP-D2 and a name→ordinal map is exactly what it should hold. Real exception ⇒ `hot-path-gate: ok — <reason>` | pre-commit `--staged`; self-tested by `--self-test`; bite-proven against the real `domain.rs` |
| [scripts/zero-digest-gate.py](../../scripts/zero-digest-gate.py) | **A ruleset pin must not be an anonymous zero** — bans an inline `RulesetDigest([0u8; 32])` anywhere, `RulesetDigest::UNPINNED` outside the kernel harness (`crates/sim`) and test files, **and a 64-zero `ruleset_digest` in TypeScript** (`'0'.repeat(64)` / the literal) — `contracts/game-wire/common.schema.json` says *the client caches BY digest*, so a shared zero gives every reality one cache key. Root cause it guards: RLS-A13 makes the digest the thing that ties an event to the rules that produced it, and an all-zero digest makes that pin **inert** — two realities with different rules stamp indistinguishable events and replay cannot detect the rules moved underneath it (IMP-D6; it stood in **15 places**, four of them production). It survived because it LOOKS like a value: nothing distinguished *“the loader is not wired yet”* from *“this domain genuinely has no ruleset”*, so both were spelled the same way. The second case is now the NAMED `UNPINNED` — the same move as `MAX_HIT` in the damage chain, a declared zero rather than an emergent one. Real exception ⇒ `zero-digest-gate: ok — <reason>` | pre-commit `--staged`; self-tested by `--self-test` |
| [scripts/envelope-mirror-gate.py](../../scripts/envelope-mirror-gate.py) | **The event envelope is ONE shape in TWO languages** — `contracts/events/envelope.go` (SSOT) and `crates/dp-kernel/src/envelope.rs` (mirror) must declare the same JSON field names in the same order. Root cause it guards: both files **state** the rule in their own doc comments (*“Adding/removing/renaming a field here REQUIRES a paired change”*) and **nothing checked it** — three of the four parts of rule+SoT+gate+test. The drift it permits is silent by construction: `serde` ignores unknown fields and Go tolerates missing ones, so a field added on one side is simply dropped downstream with no test failing. Checks names + order, deliberately **not types** (`uuid.UUID`/`Uuid`, `time.Time`/`String` are the same wire shape spelled differently; a type table would be a second contract needing its own gate) | pre-commit `--staged` (runs only when a side is touched); self-tested by `--self-test` |
| [scripts/gate-wiring-gate.py](../../scripts/gate-wiring-gate.py) | **Every gate is reachable, and every failing gate is acknowledged BY NAME.** Two checks over a *predicate* (`*-gate`/`*-lint`, both separators) so a gate written tomorrow is in scope the day it lands: (a) each is reachable from `.githooks/pre-commit` or a workflow — matched by path **or bare stem**, because `lint-foundation.yml` drives matrices of bare names and a path-only search under-reports; (b) `--run-all` executes them all, with `KNOWN_RED` rows that must name a tracked deferral and that **FAIL when the gate turns green**, so the acknowledgement list shrinks instead of becoming permanent. Root cause it guards: on 2026-07-29 six `lint-foundation` legs had been RED **on `main`** for four days — blocking, reported, ignored. The mechanism was never broken; nobody read the output. Scope limit stated in the file: the 12 `*-smoke.sh` scripts are OUT (all need a live stack) — tracked, not covered | pre-commit (wiring only, ~130ms) + `.github/workflows/gates.yml` (`--run-all`); self-tested by `--self-test`, which asserts the scope is still a predicate and that this gate is itself wired |
| [scripts/design-lint.py](../../scripts/design-lint.py) | Layer-1 design-corpus integrity (`docs/03_planning/LLM_MMO_RPG/`): broken relative links · phantom "(registered …)" claims vs the ownership matrix · stable-ID prefixes vs the id catalog · **count-drift** — a tight "N variants of `X`" claim verified against the real Rust enum (the doc-side twin of `closed-set-gate`; INFO-only until 2026-07-28). Root cause it guards: *"documents are locked individually; correctness is a property of the set"* (`19` §14) | pre-commit `--staged --warn-check symbol` (link + phantom = **hard fail**; unregistered-prefix = warn); [README](../../scripts/design-lint.README.md) |
...
| `scripts/no-absolute-host-paths.py` (pre-commit) | **No absolute HOST path in committed code.** Such a path resolves on one machine; elsewhere the script dies or — worse — the path exists and silently reads a STALE COPY from a sibling checkout (found 2026-07-23: four POC exporters pointed at a different repo, `lore-weave-mvp`, and an i18n rewriter wrote through `d:/Works/source/lore-weave/...`). In-repo target ⇒ derive from `__file__`; external target ⇒ `os.environ.get(VAR, default)` **and fail loudly when absent** — a silent wrong path is the failure mode. |

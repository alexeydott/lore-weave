...
| K13 | **What K8 was hiding.** The architecture leans on tool-level idempotency for repeated Tier-A writes, but coverage is unverified: **135 Tier-A tools, 24 create-shaped, only 2 confirmed idempotent** (`book_chapter_create`, `glossary_propose_entities`). A non-idempotent Tier-A create repeated identically across iterations writes up to 5 rows before the human gate — and this session MEASURED the model doing exactly that repeat despite an explicit success result. Needs a per-tool idempotency audit of the other 22. |

...
| K13 | **CONFIRMED BY LIVE PROBE, partially fixed.** Called each create TWICE with byte-identical args through ai-gateway: `book_create`, `world_create`, `kg_project_create` each produced **two distinct rows**. Combined with the measured cross-iteration repeat + `TIER_A_SAME_OP_CAP=5`, one user intent could mint five duplicates with no confirm card — and `book_create` has NO agent-invocable undo. **FIXED + re-probed idempotent:** `book_create`, `world_create` (N6 pre-insert guard, case-insensitive, active-rows-only, 4 DB-gated tests). **STILL OPEN:** `kg_project_create` (re-probed: still duplicates) + the 3 `world_map_*` creates + the 9 composition creates, none live-probed yet. Static file-level scan is too coarse to settle these — the probe is the ground truth. |

...
| K13 | **LIVE-PROBED + 4 FIXED, re-probe clean.** Called each create-shaped Tier-A tool twice with byte-identical args through ai-gateway. **Confirmed duplicating: `book_create`, `world_create`, `kg_project_create`, `composition_arc_create`** — all four now re-probe IDEMPOTENT (N6-shaped pre-insert guard on the natural key; live rows only; no DB unique, since legit same-name siblings exist). `kg_project_create`'s guard lives on the TOOL, not the shared repo, because `create_or_get` deliberately always-inserts on the book-less path to keep the FE 'new project' UX. **Probe hardening mattered:** the first run reported `composition_arc_create` IDEMPOTENT because it echoed the input `book_id`; excluding input ids flipped it to DUPLICATE. **Not settled (fixture gaps, not clean bills):** `world_map_create`/`add_marker`/`add_region` (world_create succeeds then map create says 'world not found' — see K14), `composition_create_work`/`canon_rule_create`/`outline_node_create` (fixture 'not found'), `composition_motif_create`/`structure_template_create` (inconclusive — probe could not read their id shape). |
| K14 | `world_map_create` reports **'world not found'** for a world `world_create` just returned (same user, same session, live-reproduced 3×) | Surfaced by the K13 probe. Either a scope/lookup mismatch between the two tools or a visibility gap — unexamined. It also BLOCKS probing the three `world_map_*` creates. |
...
| K12 | `sdks/python/tests` carries **7 pre-existing failures** (extraction dynamic-schema/prompts/resolve-config, video-gen error classes) | A second red baseline, same measurement-corrupting class as chat-service's was. Unrelated to this branch's work; needs its own depth-first pass. |

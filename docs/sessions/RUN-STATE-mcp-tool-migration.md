...
| K13 | **7 CONFIRMED DUPLICATING, ALL 7 FIXED, final re-probe `DUPLICATING: none`.** `book_create`, `world_create`, `kg_project_create`, `composition_arc_create`, `world_map_create`, `world_map_add_marker`, `world_map_add_region` — each live-probed minting a duplicate on a byte-identical double-fire; each now returns the SAME id. Guards are N6-shaped (pre-insert lookup on the natural key, live rows only, no DB unique since legit same-name siblings exist); `kg_project_create`'s sits on the TOOL so the FE's always-insert path is untouched; the marker guard keys on the FULL placement (label+x+y) so the same label elsewhere still creates. **Two probe bugs changed results mid-audit** — echoed input ids faked an IDEMPOTENT verdict, and positional id-picking faked K14 — so the harness needed hardening twice before its output could be trusted. **STILL UNSETTLED (fixture gaps, not clean bills):** `composition_create_work` / `canon_rule_create` / `outline_node_create` (fixture 'not found') and `motif_create` / `structure_template_create` (probe could not read their id shape). |

...
| S5 | **Pay the coverage debt** — kill-switches untested in either state | **PARTIAL + GATED** | Audited 15 boolean flags: **8 had zero test coverage**. 2 fully covered (mutation-proven), 7 tracked in `_UNCOVERED_DEBT` with reasons, and a **gate** now fails any NEW untested flag (proven by adding one). chat-service 1870 pass / 0 fail. |

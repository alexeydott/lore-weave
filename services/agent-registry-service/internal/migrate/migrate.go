...
INSERT INTO workflows (tier, slug, title, description, surfaces, inputs, steps, notes_md, status, source) VALUES
  ('system','chapter-compose','Draft this chapter',
   ...
   E'Use this when the user wants a specific chapter written — "draft chapter 3", "write this next part", "put down a first version of this scene". The job: know which chapter, then write its draft prose.\n\nEXACTLY WHICH TOOL DOES WHAT:\n- book_list (kind=chapters) — if you are not sure which chapters exist or what they are called, list them first to see their NUMBERS and TITLES. (If the user already named the chapter — "chapter 1" — or the book has only one, you can skip straight to writing.)\n- book_chapter_save_draft — write the chapter''s DRAFT prose. The backend RESOLVES THE CHAPTER FOR YOU: give the chapter as its NUMBER (e.g. "1") or its TITLE, or omit it when the book has one chapter. You do NOT look up a chapter id, and you do NOT read or pass a version first — just the chapter and the prose. NEVER guess a chapter id and NEVER reuse the book id or the project id as a chapter id. It returns the chapter''s new state (title, number, new version, word_count) — read that back to the user.\n\nDo NOT reach for book_update_details to "write" — that edits the BOOK''s description/blurb, not a chapter''s story text. Chapter prose ALWAYS goes through book_chapter_save_draft.\n\nSPEAK PLAINLY: say "chapter" and "draft". Tell the user what happened in their terms ("drafted the opening of chapter 1 — have a read and tell me what to change") and never claim a chapter is written before book_chapter_save_draft returns success.',
   'published','system')
ON CONFLICT (slug) WHERE tier = 'system' DO UPDATE SET
  title = EXCLUDED.title, description = EXCLUDED.description, surfaces = EXCLUDED.surfaces,
  inputs = EXCLUDED.inputs, steps = EXCLUDED.steps, notes_md = EXCLUDED.notes_md,

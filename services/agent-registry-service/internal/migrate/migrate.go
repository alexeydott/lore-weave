...
INSERT INTO workflows (tier, slug, title, description, surfaces, inputs, steps, notes_md, status, source) VALUES
  ('system','chapter-compose','Draft this chapter',
   ...
   '{book,editor,studio}'::text[], '{}'::jsonb,
   '[
     {"id":"list-chapters","tool":"book_list","gate":"none"},
     {"id":"outline","tool":"composition_get_outline_node","gate":"none"},
     {"id":"get-chapter","tool":"book_get_chapter","gate":"none"},
     {"id":"draft","tool":"book_chapter_save_draft","gate":"none","repeat":true}
   ]'::jsonb,
   E'Use this when the user wants a specific chapter written — "draft chapter 3", "write this next part", "put down a first version of this scene". The job: FIND the real chapter, read what it is meant to cover, read the chapter itself, then write its draft.\n\nEXACTLY WHICH TOOL DOES WHAT:\n- book_list (kind=chapters) — FIRST, list the book''s chapters to get the REAL chapter_id. NEVER guess a chapter id, and NEVER reuse the book id or the project id as a chapter id — they are DIFFERENT things; a wrong id fails with "no active chapter with that chapter_id", and retrying with another guess fails the same way. If you do not already hold the exact chapter_id from this list, you are not allowed to call book_get_chapter yet.\n- composition_get_outline_node — read what this chapter is meant to cover (its place in the plan). If the chapter has no place in the plan yet, say so and offer to plan it first; do not invent a chapter with no anchor.\n- book_get_chapter — read the target chapter (by the chapter_id from book_list) to get its CURRENT draft version. You MUST pass that chapter''s own draft_version as base_version to the next step — NOT the outline''s version (they are different things); passing the wrong one fails with a version conflict.\n- book_chapter_save_draft — write the DRAFT text for the chapter, using the draft_version you just read. It saves a draft, not a published chapter, and returns the new version on success.\n\nORDER MATTERS: list to get the real id, read the plan, read the chapter for its version, THEN draft — skipping the list leaves you guessing the id and looping on "no active chapter".\n\nSPEAK PLAINLY: say "chapter" and "draft", never "outline node"/"draft version"/"compose". Tell the user what happened in their terms ("drafted the opening of chapter 3 — have a read and tell me what to change") and never claim a chapter is written before its text is saved.',
   'published','system')
ON CONFLICT (slug) WHERE tier = 'system' DO UPDATE SET
  title = EXCLUDED.title, description = EXCLUDED.description, surfaces = EXCLUDED.surfaces,
  inputs = EXCLUDED.inputs, steps = EXCLUDED.steps, notes_md = EXCLUDED.notes_md,

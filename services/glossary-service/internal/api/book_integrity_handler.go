...
func (s *Server) checkBookIntegrity(w http.ResponseWriter, r *http.Request) {
	...
	checks := []integrityQuery{
		...
		{code: "entity_names", severity: "warning", message: "Live entities should have a name or term attribute.", query: `SELECT COUNT(*) FROM glossary_entities e WHERE e.book_id = $1 AND e.deleted_at IS NULL AND NOT EXISTS (SELECT 1 FROM entity_attribute_values v JOIN book_attributes a ON a.attr_id = v.attr_def_id WHERE v.entity_id = e.entity_id AND a.book_id = e.book_id AND a.kind_id = e.kind_id AND a.code IN ('name', 'term'))`},

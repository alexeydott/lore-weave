  ...
  return <Dialog.Root open onOpenChange={(next) => onOpenChange(next)}><Dialog.Portal>
    ...
    <Dialog.Content className="fixed left-1/2 top-1/2 z-50 flex max-h-[88vh] w-full max-w-3xl -translate-x-1/2 -translate-y-1/2 flex-col rounded-lg border bg-background shadow-xl">
      ...
      <main className="min-h-0 flex-1 overflow-y-auto p-5">{error && <div className="mb-4 flex gap-2 rounded border border-destructive/50 bg-destructive/10 p-3 text-xs text-destructive"><AlertTriangle className="h-4 w-4 shrink-0" />{error}</div>}
        ...
        {step === 'confirm' && inspection && <section className="space-y-4 text-sm"><p>{t('epubImport.ready', { count: selected.size, title: inspection.metadata.title || file?.name })}</p><fieldset className="space-y-2"><legend className="text-xs font-medium">{t('epubImport.strategyLabel')}</legend>{(['append', 'merge_by_source_key', 'replace_all'] as const).map((item) => <label className="mr-4 text-xs" key={item}><input type="radio" checked={strategy === item} onChange={() => setStrategy(item)} /> {t(`epubImport.strategy.${item}`)}</label>)}</fieldset>{strategy === 'replace_all' && <div className="rounded border border-amber-500/40 bg-amber-500/10 p-3 text-xs">{t('epubImport.replaceWarning')}</div>}</section>}
        ...
        {step === 'report' && <section className="space-y-4"><div className="flex gap-2"><CheckCircle2 className="h-5 w-5 text-green-600" /><div><p className="text-sm font-medium">{job?.status}</p><p className="text-xs text-muted-foreground">{t('epubImport.reportSummary', { chapters: report?.chapters_created ?? job?.chapters_created ?? 0, warnings: report?.warnings?.length ?? 0 })}</p></div></div>{report?.warnings?.length ? <div className="rounded border border-amber-500/40 bg-amber-500/10 p-3 text-xs"><p className="mb-1 font-medium">{t('epubImport.warningDetails')}</p><pre className="max-h-28 overflow-auto whitespace-pre-wrap">{JSON.stringify(report.warnings, null, 2)}</pre></div> : null}{report?.errors?.length ? <pre className="max-h-28 overflow-auto rounded border bg-muted p-2 text-xs">{JSON.stringify(report.errors, null, 2)}</pre> : null}<div className="flex gap-2">{job?.resumable && <button disabled={busy} onClick={() => void updateJob('resume')} className="rounded border px-3 py-1.5 text-xs"><RotateCcw className="mr-1 inline h-3 w-3" />{t('epubImport.resume')}</button>}{job?.rollback_available && <button disabled={busy} onClick={() => void updateJob('rollback')} className="rounded border border-destructive/50 px-3 py-1.5 text-xs text-destructive">{t('epubImport.rollback')}</button>}{targetMode === 'new_book' && job?.book_id && <button onClick={() => navigate(`/books/${job.book_id}`)} className="rounded border px-3 py-1.5 text-xs">{t('epubImport.openBook')}</button>}<button onClick={() => onOpenChange(false)} className="rounded bg-primary px-3 py-1.5 text-xs text-primary-foreground">{t('epubImport.close')}</button></div></section>}
      </main>
      {(step === 'metadata' || step === 'chapters' || step === 'confirm') && <footer className="flex justify-between border-t p-3"><button className="rounded border px-3 py-1.5 text-xs" onClick={() => setStep(STEPS[Math.max(0, currentIndex - 1)])}>{t('epubImport.back')}</button><button disabled={busy || (step === 'chapters' && selected.size === 0)} className="rounded bg-primary px-3 py-1.5 text-xs text-primary-foreground disabled:opacity-50" onClick={() => step === 'confirm' ? void beginImport() : setStep(STEPS[currentIndex + 1])}>{step === 'confirm' ? t('epubImport.start') : t('epubImport.continue')}</button></footer>}
    </Dialog.Content>
